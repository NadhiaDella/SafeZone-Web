...
<h1 class="p-2 text-center">Form Pengajuan Bantuan Hukum</h1>
<div class="form-wrapper2">
    <form action="{{ route('store.hukum') }}" method="POST">
        @csrf
        <div class="profile">
            <img src="{{ asset('Gambar/'. $advocat->user->image) }}" alt="" class="profile-pic">
            <div>
                <h2>{{ $advocat->user->name }}</h2>
                <p class="role">{{ $advocat->title }}</p>
            </div>
        </div>
        ...
        <div class="services">
            <div class="form-group">
                ...
                <input type="hidden" name="advocat_id" value="{{ $advocat->user->id }}">
